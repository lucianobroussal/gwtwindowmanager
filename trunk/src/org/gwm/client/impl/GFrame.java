...
public class GFrame extends PopupPanel implements MouseListener, WindowResizeListener{
...
//		windowLayout.setBorderWidth(1);
	...
	private void fireWindowActivated() {
		for(int i=0; i < listeners.size();i++){
			...
			listener.windowActivated(new GFrameEvent(this));
		}
	}
	...
	private void fireWindowResized(){
		for(int i=0; i < listeners.size(); i++){
			GFrameListener listener = (GFrameListener) listeners.get(i);
			listener.windowResized(new GFrameEvent(this));
		}
	}
	private void fireWindowClosing(){
		for(int i=0; i < listeners.size(); i++){
			GFrameListener listener = (GFrameListener) listeners.get(i);
			listener.windowClosing(new GFrameEvent(this));
		}
	}
	private void fireWindowClosed(){
		for(int i=0; i < listeners.size(); i++){
			GFrameListener listener = (GFrameListener) listeners.get(i);
			listener.windowClosed(new GFrameEvent(this));
		}
	}
	private void fireWindowMaximized(){
		for(int i=0; i < listeners.size(); i++){
			GFrameListener listener = (GFrameListener) listeners.get(i);
			listener.windowMaximized(new GFrameEvent(this));
		}
	}
	private void fireWindowMinimized(){
		for(int i=0; i < listeners.size(); i++){
			GFrameListener listener = (GFrameListener) listeners.get(i);
			listener.windowMinimized(new GFrameEvent(this));
		}
	}
	private void fireWindowMoved(){
		for(int i=0; i < listeners.size(); i++){
			GFrameListener listener = (GFrameListener) listeners.get(i);
			listener.windowMoved(new GFrameEvent(this));
		}
	}
