...
public class DefaultGFrame extends SimplePanel implements GFrame, EventPreview {
	...
	private List<GFrameListener> listeners;
	...
	private void initializeFrame() {
		...
		listeners = new ArrayList<GFrameListener>();
	...
	protected void buildGui() {
		...
		ui.getCellFormatter().setAlignment(1, 0, HasHorizontalAlignment.ALIGN_CENTER, HasVerticalAlignment.ALIGN_MIDDLE);
		ui.setCellPadding(0);
		ui.setCellSpacing(0);
	...
	private Widget getFrame() {
		if (frame == null) {
			frame = new HTML("<iframe src='" + url + "' width=100% height=100% frameborder=0></iframe>");
			frame.setWidth("100%");
			frame.setHeight("100%");
	...
	public void fireFrameResized() {
		for (int i = 0; i < listeners.size(); i++) {
			GFrameListener listener = listeners.get(i);
	...
	public void fireFrameMoving() {
		for (int i = 0; i < listeners.size(); i++) {
			GFrameListener frameListener = listeners.get(i);
	...
	public void setPixelSize(int width, int height) {
		throw new IllegalStateException("Should never be called. Instead use public void setSize(int width, int height){...}");
	}
